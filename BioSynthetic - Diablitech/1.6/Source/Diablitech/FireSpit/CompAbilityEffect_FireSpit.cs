...
{
    class CompAbilityEffect_FireSpit : CompAbilityEffect
    {
        private readonly List<IntVec3> tmpCells = new List<IntVec3>();

        private new CompProperties_AbilityFireSpit Props => (CompProperties_AbilityFireSpit)props;

        private Pawn Pawn => parent.pawn;

        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            Need_Food need_Food = parent.pawn.needs?.TryGetNeed<Need_Food>();
            if (need_Food != null)
            {
                if (need_Food.CurLevelPercentage > 0f)
                {
                    need_Food.CurLevel -= need_Food.MaxLevel * 0.333f;
                }
            }
            ...
            if (parent.pawn.health.hediffSet.TryGetHediff(HediffDefOf.FoodPoisoning, out Hediff foodPoisoningHediff))
            {
                if(foodPoisoningHediff != null)
                {
                    //parent.pawn.health.hediffSet.GetMostRecentHediff
                    parent.pawn.health.RemoveHediff(foodPoisoningHediff);
                }
            }
